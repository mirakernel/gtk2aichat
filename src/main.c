...
static void build_ui(App*a){
    ...
    GtkWidget *right=gtk_vbox_new(FALSE,0),*viewbar=gtk_hbox_new(FALSE,5),*chat=gtk_hpaned_new(),*conversation=gtk_vbox_new(FALSE,0),*compose_outer=gtk_vbox_new(FALSE,4),*compose=gtk_vbox_new(FALSE,4),*compose_actions=gtk_hbox_new(FALSE,5),*meta=gtk_hbox_new(FALSE,8),*inspector=gtk_vbox_new(FALSE,7);
    ...
    a->input=gtk_text_view_new();gtk_text_view_set_wrap_mode(GTK_TEXT_VIEW(a->input),GTK_WRAP_WORD_CHAR);is=scroll(a->input);gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(is),GTK_SHADOW_IN);gtk_widget_set_size_request(is,-1,64);gtk_box_pack_start(GTK_BOX(compose),is,FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(compose_actions),attach,FALSE,FALSE,0);gtk_box_pack_start(GTK_BOX(compose_actions),emoji,FALSE,FALSE,0);a->send=gtk_button_new_with_label("Отправить");gtk_box_pack_end(GTK_BOX(compose_actions),a->send,FALSE,FALSE,0);gtk_box_pack_start(GTK_BOX(compose),compose_actions,FALSE,FALSE,0);
    ...
    gtk_box_pack_start(GTK_BOX(meta),a->attachment_count,FALSE,FALSE,0);gtk_box_pack_start(GTK_BOX(meta),a->agent_label,TRUE,TRUE,0);label=gtk_label_new("16 384 ток.");gtk_box_pack_end(GTK_BOX(meta),label,FALSE,FALSE,0);gtk_box_pack_start(GTK_BOX(compose_outer),compose,FALSE,FALSE,0);gtk_box_pack_start(GTK_BOX(compose_outer),meta,FALSE,FALSE,0);gtk_container_set_border_width(GTK_CONTAINER(compose_outer),7);gtk_box_pack_start(GTK_BOX(conversation),gtk_hseparator_new(),FALSE,FALSE,0);gtk_box_pack_start(GTK_BOX(conversation),compose_outer,FALSE,FALSE,0);
