...
static void test_path_sandbox(void) {
    ...
    result=agent_tool_execute("read_file",args,root,TRUE,FALSE);
    g_assert_true(g_str_has_prefix(result,"ERROR:"));
    g_free(result);json_object_put(args);
    g_assert_cmpint(symlink("/etc/passwd",link),==,0);
    args=args1("path","outside-link");result=agent_tool_execute("read_file",args,root,TRUE,FALSE);
    g_assert_true(g_str_has_prefix(result,"ERROR:"));
...
static void test_read_and_search(void) {
    ...
    args=args1("path","hello.txt");result=agent_tool_execute("read_file",args,root,TRUE,FALSE);
    g_assert_nonnull(strstr(result,"2: needle"));g_free(result);json_object_put(args);
    args=args1("query","needle");result=agent_tool_execute("search_files",args,root,TRUE,FALSE);
...
static void test_write_permissions_and_replace(void) {
    ...
    result=agent_tool_execute("write_file",args,root,TRUE,FALSE);
    g_assert_true(g_str_has_prefix(result,"ERROR:"));g_free(result);
    result=agent_tool_execute("write_file",args,root,TRUE,TRUE);
    ...
    result=agent_tool_execute("replace_in_file",args,root,TRUE,TRUE);
    g_assert_true(g_str_has_prefix(result,"OK:"));g_free(result);json_object_put(args);
...
static void test_schema(void) {
    ...
    g_assert_cmpuint(json_object_array_length(schema),==,7);
...
static void test_skills(void) {
    ...
    args=json_object_new_object();result=agent_tool_execute("list_skills",args,root,TRUE,FALSE);
    g_assert_nonnull(strstr(result,"demo"));g_assert_nonnull(strstr(result,"Demo skill"));g_free(result);json_object_put(args);
    args=args1("name","demo");result=agent_tool_execute("read_skill",args,root,TRUE,FALSE);
